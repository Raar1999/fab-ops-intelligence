...
#:
#: 1.0.0 -> 1.1.0 for the row-order correction, although the reports on the
#: library are byte-for-byte unchanged (measured: zero differing values across
#: five scenarios, down to the float hex). The rule says "could", and the two
#: implementations differ in exactly that: before, a report was a function of
#: the data *and* of whatever order the query planner returned rows in; after,
#: it is a function of the data. A database vacuumed, re-indexed or written by
#: a different SQLite could have separated them, and a report that carried one
#: version number for both would have been unable to say which engine produced
#: it. This follows ADR-023's precedent on the other side of the boundary,
#: where the generator's version moved 0.6.0 -> 0.7.0 on an emission change
#: that altered no generated byte.
#: The Phase 6 benchmark re-measured the anchor rule, the statistic and the
#: level against a twelve-scenario library and 660 fault-free worlds (ADR-031).
#: None of the three moved, so no further bump: the engine that ships after
#: that gate computes what the engine before it computed, and a version that
#: moved without generation moving would cost a reviewer the one signal a
