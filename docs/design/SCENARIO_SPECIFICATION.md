...
- **Expected evidence:** elevated edge-zone defect share, edge-site CD deviation, temporal alignment of both with the window, repair→improvement — and a chamber-grain yield split (≈ 4–8 pts, vs σ≈3 wafer noise) that is ***retired as a binding expectation and kept as reported evidence*** (ADR-027 §6 for the magnitude, **ADR-028** for the attribution). Three measurements settle it: the parametric channel's whole budget on a healthy fab is 0.80 yield points; the audited v1's own mediated remainder was 3.7, below this band's floor; and on twelve fault-free worlds each etch tool is "worst on cohort yield" about a third of the time, so the ranking is satisfied by chance on a world with no fault in it. Yield remains in the simulator, the schema and the queries — it is a downstream consequence here, not the channel this scenario is attributed through. See `CAUSAL_MECHANISM_MODEL.md` §8.
