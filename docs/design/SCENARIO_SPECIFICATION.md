...
- **Expected evidence:** chamber-grain yield split (≈ 4–8 pts, vs σ≈3 wafer noise — ***retired as a binding expectation by ADR-027 §6** and left as a historical reference: the parametric channel's whole budget on a healthy fab is 0.80 yield points, the audited v1's own mediated remainder was 3.7, and the between-tool benign spread on cohort yield (0.41 pts) exceeds the mechanism's contribution (0.058) by seven times. See `CAUSAL_MECHANISM_MODEL.md` §8*), elevated edge-zone defect share, edge-site CD deviation, temporal alignment of all three with the window, repair→improvement.
