...
## 3.8 L7, restated (the architecture decision gate)

That gate ran and chose. **ADR-027** replaces L7's undeclared constant with the distribution of its own statistic under the null hypothesis it is about, and splits the criterion into the two questions it was conflating. The table's pass criterion above is superseded by this section.

**The reference.** `src/fabeval/reference.py` simulates worlds whose chambers differ by *nothing at all* — the exchangeability F10/F11 make true by construction — and reports two orderings of `|leave-one-out z|`: `per_chamber`, for one *specified* chamber, and `per_world`, for the *worst* one. It is derived from the chamber count and from nothing any dataset contains, which is what stops it being a threshold fitted to the data it judges.

**Clause 1 — the per-world action limit.** On a null, no chamber may exceed the per-chamber critical value at the fab's **own** control-limit convention: 3 sigma, the multiple eight of the nine `alarms.codes` declare, i.e. a false-alarm rate of 0.0027, which is **6.46** at seven chambers. An action limit rather than a screening one because this runs on every null of every build — the same reason a real fab charts at 3 sigma and not at 2 — and the convention is borrowed from the simulated fab rather than invented by the evaluator.

**Clause 2 — the population calibration.** Cross-dataset, like L8. Every chamber of every fault-free world on every reference channel, against the per-chamber critical value at the declared screening level `ALPHA = 0.05`; the realized exceedance rate must not exceed that level by more than chance. This is the half with power against the failure a *generator* defect would actually produce — structure spread thinly, where every world is a little out and none looks alarming. It refuses fewer than three fault-free worlds, and it reports the smallest inflation the sample it was given can resolve, because a calibration check that cannot say how blind it is will be read as though it were not.

**Measured.** Zero of twelve fault-free worlds trip the action limit (worst chamber seen 5.38 against 6.46). The pooled exceedance rate is 9/252 = 0.036 against 0.050 expected — and 0/252 at the fab's stricter 3-sigma level, so the conclusion does not depend on which level is declared.

**What the correction costs, stated rather than glossed.** Poisoning one chamber's `cd_nm_edge` in a null database: 30% is caught at 19.2 sigma, 10% at 10.8, and **5% passes at 4.7**. The old constant caught 2%. It also flagged nine healthy worlds in ten, and at 2% it named the *wrong* chamber — so what was given up is not sensitivity but a check that fired on the benign structure rule F11 requires the null to contain. `A7` moves **BLOCKED → PARTIAL**; the mutation test still fails on a poisoned null, and `tests/fabeval/test_reference.py` pins the derivation of every number above.
