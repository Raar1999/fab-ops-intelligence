...
- **Maintenance reaches latent state, but only PMs do — so far.** Step 3A implements §3's PM effects on the latent plane (`CAUSAL_MECHANISM_MODEL.md` §6): a PM cleans `particle_load` completely, recentres `param_bias` by a drawn fraction, and leaves `edge_uniformity` and the benign offsets alone. The **fault-driven repairs** of §2.3 above — alarm escalation, `repair_delay`, recovery fraction — are Step 3B. One requirement carries forward with them: background breakdowns must move latent state too, or "a repair that changed behaviour" would be a fault fingerprint, since only faulted chambers would ever have caused one.
