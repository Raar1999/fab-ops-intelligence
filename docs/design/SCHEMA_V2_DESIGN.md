...
- The Phase 2 semantic layer will target v2; compat views mapping the v1 analytical surface (e.g., `v_gate_etch_runs` semantics) onto v2 are a Phase 2 deliverable, not Phase 1.
  > **As implemented (2026-08-10, ADR-032).** The layer targets v2 as designed and lives at `src/fabops/semantic/`, opened read-only with its views as `TEMP` views so that reading a dataset cannot change the bytes its manifest hashes. **The compat views were deliberately not built.** They assumed the v1 surfaces would migrate; ADR-010 keeps them on the v1 database, so a compat view on v2 would have had no consumer, and a view with no consumer is dead surface. What the compat clause existed to preserve — addressing a step by name rather than by `step_id` — is delivered by the spine carrying `step_name`.
